...
module.exports = class Service extends Base {
  _find (params) {
    ...
    const { customer, ...rest } = filtered.stripe;
    if (!customer) {
      debug('Missing Stripe customer id');
    }
    if (!filtered.query.type) {
      filtered.query.type = 'bank_account';
    }
    return this.handlePaginate(
      filtered,
      this.stripe.customers.listSources(customer, filtered.query, rest)
    );
  }
  ...
  _update (id, data, params) {
    ...
    const { customer, ...rest } = stripe;
    return this.stripe.customers.updateCard(customer, id, data, rest);
  }
