...
module.exports = class BaseService extends AdapterService {
  ...
  filterParams (params = {}) {
    return {
      ...
      paginate: params.paginate === false ? false : true
    };
  }

  handlePaginate ({ paginate, query, stripe }, stripeMethod) {
    if (paginate) {
      return stripeMethod;
    }
    if (stripeMethod.autoPagingToArray) {
      return stripeMethod
        .autoPagingToArray({ limit: query.limit }, stripe.onDone);
    }
